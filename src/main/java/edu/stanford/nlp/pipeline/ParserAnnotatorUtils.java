...
public class ParserAnnotatorUtils  {
  ...
  /**
   * This method is added to bridge between CoreNLP 3.5.1, which is used in the Clulab processor,
   *   and CoreNLP 3.7.0 which is the latest version at the time of writing.
   * @param verbose
   * @param buildGraphs
   * @param gsf
   * @param sentence
  ...
  public static void fillInParseAnnotations(boolean verbose, boolean buildGraphs, 
			...
			Tree tree, GrammaticalStructure.Extras extras) {

	  	if (verbose)
	  		log.info("Repackaging old 3.5.1 CoreNLP call into the matching 3.7.0 CoreNLP call...");
		ArrayList<Tree> trees = new ArrayList<>();
		trees.add(tree);
		fillInParseAnnotations(verbose, buildGraphs, gsf, sentence, trees, extras);
